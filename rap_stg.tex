...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Odélia Guedj}
\fancyhead[C]{Université Paris Saclay - INSERM U970 E04}
\fancyhead[R]{M1 MINT}
...
\noindent
\begin{tikzpicture}
\draw (0,0) -- (2,0) -- (2,0.3) -- (0,0.3) -- (0,0);
\end{tikzpicture} : Taille population d'étude, exclusions\\

\noindent
\begin{tikzpicture}
\draw (0,0) ellipse (1cm and 0.15cm);
\end{tikzpicture} : Type d'analyse\\

\noindent
\begin{tikzpicture}
\draw (0,0) -- (1,.15) -- (2,0) -- (1,-.15) -- (0,0);
\end{tikzpicture} : Sous groupes\\

\noindent
\begin{tikzpicture}
\draw (2,2) circle (.3cm);
\end{tikzpicture} : Modèles \\
...
%\underset{H_0}{\sim} \chi ^2 \left(n-\left( p+1 \right)\right)
Statistique de test : \\
\begin{align*}
...
%\left( p+1 \right) \left( 1 - {\left[ W^{1 \over 2} X {\left(X^t WX \right) }^{-1} X^t W^{1 \over 2} \right]_{i,i}} 
%{\left(e_i^{textsuperscript{*}}\right)}^2
...
\newpage
\includegraphics[scale=.29]{quest_ipc-8.png}
